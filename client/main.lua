...
-- NUI Management
local nuiOpen = false
local nearRentalNPC = false
local canRentVehicle = false
local canReturnVehicle = false
...
local localeStrings = {
    ...
    error_no_active_job = "You need to start a job first",
...
-- NUI Functions
local function openTruckerNUI(fromNPC)
    if nuiOpen then return end
    
    -- Check if opened from NPC interaction or F6
    canRentVehicle = fromNPC or false
    canReturnVehicle = fromNPC or false
...
RegisterNUICallback('rentVehicle', function(data, cb)
    if not canRentVehicle then
        SendNUIMessage({
            action = "showNotification",
            message = "You need to interact with the rental NPC to rent a vehicle",
            type = "error"
        })
        cb('error')
        return
...
RegisterNUICallback('startJob', function(data, cb)
    if not jobActive then
        SendNUIMessage({
            action = "showNotification",
            message = "You need to rent a vehicle first",
            type = "error"
        })
        cb('error')
        return
    ...
    -- Job is already started when vehicle is rented, just update UI
    updateNUIJobStatus()
    cb('ok')
end)
...
RegisterNUICallback('returnVehicle', function(data, cb)
    if not canReturnVehicle then
        SendNUIMessage({
            action = "showNotification",
            message = "You need to interact with the rental NPC to return a vehicle",
            type = "error"
        })
        cb('error')
        return
...
-- F6 Key Handler
RegisterCommand('+trucker_menu', function()
    if not jobActive then
        notify("You need to start a job first by talking to the rental NPC", "error")
        return
    end
    
    openTruckerNUI(false) -- false = not from NPC
end, false)

RegisterCommand('-trucker_menu', function()
    -- Command release handler (not needed for this)
end, false)

RegisterKeyMapping('+trucker_menu', 'Open Trucker Job Menu', 'keyboard', 'F6')
...
-- Main NPC creation with all functions
local function createMainNPC()
    if npcSpawned then return end
    
    local loc = sharedConfig.locations.npc
    ...
    npcPed = CreatePed(4, model, loc.coords.x, loc.coords.y, loc.coords.z - 1.0, loc.coords.w or loc.heading, false, true)
    ...
    if exports and exports.ox_target then
        exports.ox_target:addLocalEntity(npcPed, {
            {
                ...
                distance = 3.0,
                onSelect = function()
                    openTruckerNUI(true) -- true = from NPC
                end,
            ...
            
            {
                ...
                label = localeStrings.info_pickup_paycheck,
                distance = 3.0,
                onSelect = function()
                    if lib and lib.progressBar then
                        local success = lib.progressBar({
                            duration = 2000,
                            label = 'Collecting paycheck...',
                            useWhileDead = false,
                            canCancel = false,
                            disable = {
                                car = true,
                        ...
                        if success then
                            ...
                            notify("Paycheck collection cancelled.", "error")
                        end
                    else
                        getPaid()
                    end
                ...
                canInteract = function()
                    return completedDeliveries > 0
                end
            },
            
            {
                name = 'trucker_return_vehicle',
                icon = 'fas fa-undo',
        ...
        debugPrint("✅ Main NPC target interactions added successfully")
    else
        debugPrint("⚠️ ox_target not available, NPC interactions disabled")
    end

    npcSpawned = true
    debugPrint("🚛 Main NPC spawned with all functions")
end

-- Enhanced delivery location NPCs with box pickup animation
local function createDeliveryNPC(location)
    if not location.coords then return nil end
        ...
        if DoesEntityExist(npcPed) then
            ...
            if exports and exports.ox_target then
                exports.ox_target:addLocalEntity(npcPed, {
                    ...
                    icon = 'fas fa-box',
                    label = ('Deliver to %s'):format(location.label or 'Location'),
                    distance = 2.5,
                    onSelect = function()
                        local ped = PlayerPedId()
                        ...
                        -- Enhanced delivery with box pickup animation
                        if lib and lib.progressBar then
                            local success = lib.progressBar({
                                duration = 3000,
                            ...
                            if success then
                                -- NPC picks up the box animation
                                local npcPedId = npcPed
                                RequestAnimDict('anim@heists@box_carry@')
                                while not HasAnimDictLoaded('anim@heists@box_carry@') do
                                    Wait(10)
                                end
                                
                                TaskPlayAnim(npcPedId, 'anim@heists@box_carry@', 'idle', 8.0, -8.0, -1, 49, 0, false, false, false)
                                
                                -- Box disappears after 5 seconds
                                Citizen.SetTimeout(5000, function()
                                    ClearPedTasks(npcPedId)
                                    notify("Package delivered and processed!", "success")
                                end)
...
local function deleteNPC()
    if npcPed and DoesEntityExist(npcPed) then
        if exports and exports.ox_target then
            exports.ox_target:removeLocalEntity(npcPed, {
                'trucker_npc_talk',
                'trucker_collect_paycheck',
                'trucker_return_vehicle'
            })
        end
        DeleteEntity(npcPed)
        npcPed = nil
    ...
    npcSpawned = false
    debugPrint("🧹 Main NPC cleaned up")
end
...
function getNewLocation(locationIndex, drop)
    ...
    -- Spawn delivery NPC with enhanced animation
    currentLocation.npcPed = createDeliveryNPC(store)
...
function assignDelivery(poolIndex, boxCount)
    ...
    -- Spawn NPC with enhanced delivery functionality
    currentLocation.npcPed = createDeliveryNPC(location)
...
RegisterCommand('truckerdebug', function()
    ...
    debugPrint("Near Rental NPC:", nearRentalNPC)
    debugPrint("Can Rent Vehicle:", canRentVehicle)
    debugPrint("Can Return Vehicle:", canReturnVehicle)
